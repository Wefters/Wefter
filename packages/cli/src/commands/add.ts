...
export async function add(
  ...
  if (!validation.valid) {
    return { added: false, alreadyDeclared: false, issues: validation.issues };
  }
  ...
  if (existingPlugins.includes(name)) {
    return { added: false, alreadyDeclared: true, issues: [] };
  }
  ...
  return { added: true, alreadyDeclared: false, issues: [] };
}
